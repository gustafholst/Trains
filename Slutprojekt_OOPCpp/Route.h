...
public:
	Route() = default;
	Route(const Route &route) { *this = route; }
	...
	virtual std::string getDepStation() const { return m_depStation; }
	virtual std::string getArrStation() const { return m_arrStation; }
	virtual Time getDepTime() const { return m_depTime; }
	virtual Time getArrTime() const { return m_arrTime; }
	virtual int getMaxSpeed() const { return m_maxSpeed; }
	virtual std::vector<std::string> getVehicleTypes() const;
